...
!function(){if(document.documentElement.clientWidth>=740){const e=(e,t)=>{t&&(e.style.transition=`opacity ${t} ms`),e.style.opacity="0"},t=(e,t)=>{if(e.style.opacity=0,t){let s=0;const a=setInterval((function(){s+=50/t,s>=1&&(clearInterval(a),s=1),e.style.opacity=s}),50)}else e.style.opacity=1};document.addEventListener("click",s=>{const a=s.target;if(a.matches(".features__item")){const s=document.querySelector(".features"),c=a.querySelector(".features__text"),i=c.scrollHeight;a.matches(".features__item--active")?(s.classList.remove("features--expanded"),a.classList.remove("features__item--active"),c.style.height="0px",e(c,400)):(s.classList.add("features--expanded"),a.classList.add("features__item--active"),a.classList.remove("features__item--compact"),c.style.height=i+"px",t(c,400));[...a.parentNode.children].filter(e=>e!==a).forEach(t=>{s.matches(".features--expanded")?t.classList.add("features__item--compact"):t.classList.remove("features__item--compact"),t.classList.remove("features__item--active");const a=t.querySelector(".features__text");a.style.height="0px",e(a,400)})}})}else new Swiper(".features__inner",{pagination:{el:".features__dots",type:"bullets"},speed:500,loop:!0})}();
!function(){const s=document.body,o=document.querySelector(".main-nav");let l=0;window.addEventListener("scroll",()=>{const c=window.pageYOffset;c<=0?s.classList.remove("scroll-up"):(c>l&&!s.classList.contains("scroll-down")?(s.classList.remove("scroll-up"),s.classList.add("scroll-down"),o.classList.remove("main-nav--active")):c<l&&s.classList.contains("scroll-down")&&(s.classList.remove("scroll-down"),s.classList.add("scroll-up")),l=c)})}();
!function(){const e=document.querySelector(".main-nav");document.querySelector(".main-nav__toggle").addEventListener("click",()=>{e.matches(".main-nav--active")?e.classList.remove("main-nav--active"):e.classList.add("main-nav--active")}),document.addEventListener("click",()=>{e.classList.remove("main-nav--active")}),e.addEventListener("click",e=>{e.stopPropagation()})}();
!function(){const e=e=>{const t=e.match(/(\d{4})-(\d{2})-(\d{2})/);return{year:t[1],month:t[2],day:t[3]}},t=document.getElementById("year"),a=document.getElementById("month");t.addEventListener("change",e=>{"all"===t.value?(a.disabled=!0,a.selectedIndex=0):a.disabled=!1,d(e.target)}),a.addEventListener("change",e=>{d(e.target)});const n=document.querySelector(".news__not-found"),s=document.querySelector(".more");s.addEventListener("click",e=>{s.matches(".loading")||d(e.target)});const d=d=>{const l=s.innerHTML;d===s&&(s.classList.add("loading"),s.textContent=s.dataset.loading),fetch("js/news.json").then(e=>e.json()).then(o=>{const r=document.getElementById(s.dataset.id);let c=parseInt(s.dataset.offset);d!==t&&d!==a||(c=0);const i=parseInt(s.dataset.add);let h="";o.sort((t,a)=>{const n=e(t.date),s=e(a.date);return s.year!==n.year?s.year-n.year:s.month!==n.month?s.month-n.month:s.day-n.day});const m=(o=o.filter(n=>{const s=t.value,d=a.value,l=e(n.date),o=l.year,r=l.month;return"all"==s&&"all"==d||o==s&&"all"==d||o==s&&r==d?n:void 0})).length;(o=o.filter((e,t)=>{if(t>=c&&t<c+i)return e})).forEach((t,a)=>{const n=t.image1x,s=t.image2x,d=t.title,l=t.text,o=t.date,r=e(t.date),c=`\n            <article class="card${0==a?" js-scroll-to":""}">\n              <header class="card__head">\n                <img class="card__image" src="img/${n}" srcset="img/${s}" width="316" height="220" alt="" />\n                <h3 class="card__title">\n                  <a class="card__title-link" href="#">${d}</a>\n                </h3>\n              </header>\n              <p class="card__text">${l}</p>\n              <time class="card__date" datetime="${o}">${r.day}.${r.month}.${r.year}</time>\n            </article>\n          `;h+=c}),d!==t&&d!==a||(r.innerHTML=h,s.dataset.offset=i,s.style.display=c+i>=m?"none":"block",0===o.length?(n.style.display="block",s.style.display="none"):n.style.display="none"),d===s&&setTimeout(()=>{r.insertAdjacentHTML("beforeend",h),s.dataset.offset=c+i,s.classList.remove("loading"),s.innerHTML=l;let e=r.querySelectorAll(".js-scroll-to");e.length&&(e=e[e.length-1],e.scrollIntoView({behavior:"smooth"})),s.dataset.offset>=m&&(s.style.display="none")},700)})}}();

document.querySelectorAll(".page-nav__link").forEach(e=>{e.addEventListener("click",t=>{const l=e.getAttribute("href").replace("#",""),o=document.getElementById(l);o.style.scrollMarginTop="30px",o.scrollIntoView({behavior:"smooth"}),t.preventDefault()})});
!function(){const e=document.querySelectorAll(".slider__item").length,n=document.querySelector(".slider__text"),i=new Swiper(".slider",{init:!1,navigation:{prevEl:".slider__arrow--prev",nextEl:".slider__arrow--next"},pagination:{el:".slider__fraction",clickable:!1,renderBullet:function(n,i){return'<span class="'+i+'">'+(n+1)+" / "+e+"</span>"}},speed:500,loop:!0,grabCursor:!0,simulateTouch:!0,a11y:{prevSlideMessage:"Предыдущий слайд",nextSlideMessage:"Следующий слайд",paginationBulletMessage:"Перейти к слайду {{index}}"}});i.on("init slideChange",(function(){(e=>{let i=e.querySelector(".slider__item-text");null!==i?(i=i.innerHTML,n.innerHTML=i):n.innerHTML=""})(i.slides[i.activeIndex])})),i.init()}();